...
def go_enrichment(request):
    if request.method == 'GET':
        ...
        if any(f"{cat}_page" in request.GET for cat in ['MF', 'BP', 'CC', 'other']):
            ...
            if not gene_list:
                return render(request, 'tools/go_enrichment.html', {'error': '没有找到基因列表数据'})
            ...
            try:
                ...
                return render(request, 'tools/go_enrichment_result.html', context)
            except Exception as e:
                return render(request, 'tools/go_enrichment.html', 
                            {'error': f'分页出错: {str(e)}'})
