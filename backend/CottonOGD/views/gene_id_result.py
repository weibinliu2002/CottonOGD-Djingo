...
def geneid_result(request):
    ...
    if uuid not in UuidManager.uuid_storage:
        return Response({'error': 'uuid is required'}, status=status.HTTP_400_BAD_REQUEST)
    '''
    gene_id=request.POST.get('gene_id') or request.query_params.get('gene_id')
    genome_id=request.POST.get('genome_id') or request.query_params.get('genome_id')
    db_id=request.POST.get('db_id') or request.query_params.get('db_id')
